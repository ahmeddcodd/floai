...
const META_APP_ID_VALUE = META_APP_ID.trim();
const META_CONFIG_ID_VALUE = META_CONFIG_ID.trim();
...
export default function HomePage() {
  ...
  useEffect(() => {
    const getSession = async () => {
      ...
      if (currentUser && session) {
        try {
          const response = await fetch(`${API_BASE}/api/merchants/me`, {
          ...
          if (response.ok) {
            ...
            if (data?.merchant_id) {
              ...
              router.push(`/dashboard?merchant_id=${encodeURIComponent(data.merchant_id)}`);
              return;
            }
          }
        } catch (err) {
  ...
  useEffect(() => {
    if (!META_APP_ID_VALUE) return;

    window.fbAsyncInit = function () {
      window.FB.init({
        appId: META_APP_ID_VALUE,
    ...
    window.FB.login(
      (response) => {
        if (response.authResponse) {
          ...
          console.log("FB Login auth code:", code);
          setWaAuthCode(code);
          // waData is already set by the WA_EMBEDDED_SIGNUP postMessage listener
          setWaConnected(true);
        } else {
      ...
      {
        config_id: META_CONFIG_ID_VALUE,
        response_type: "code",
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(message || "Unable to register merchant.");
      }
      ...
      if (waAuthCode && waData?.phone_number_id && waData?.waba_id) {
        try {
          const waRes = await fetch(
            `${API_BASE}/api/merchants/${encodeURIComponent(merchantId)}/whatsapp`,
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
          ...
          if (waRes.ok) {
            console.log("WhatsApp credentials stored successfully");
          } else {
      ...
      router.push(`/dashboard?merchant_id=${encodeURIComponent(merchantId)}`);
    } catch (err) {
        ...
        >
          <div className="split">
            ...
            <form onSubmit={handleSubmit} className="form-grid">
              ...
              {META_APP_ID_VALUE && META_CONFIG_ID_VALUE && (
                  ...
                  >
                    {waConnected ? (
                      ...
                      >
                        ...
                        {fbReady ? "Connect WhatsApp" : "Loading SDK..."}
                      </button>
                    )}
                  </div>
